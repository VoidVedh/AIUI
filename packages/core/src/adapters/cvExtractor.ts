...
export class CvExtractor {
  ...
  public static async extractFromImageBuffer(
    ...
    if (!png || !png.data || png.data.length === 0) {
      ...
      (doc.metadata as any).isApproximate = true;
      (doc.metadata as any).perceptionMode = "offline-fallback-unparseable";
      return doc;
    }
   ...
   */
  private static analyzePixelBuffer(png: PNG) {
   ...
   */
  public static buildTwoPanelSplit(
    nodes: Record<string, UINode>,
    rootId: string,
    colors: ExtractedColorSummary,
    leftDark: boolean,
    viewport: { width: number; height: number },
    customCopy?: { heroHeadline?: string; heroSubtitle?: string; formTitle?: string }
  ) {
  ...
  private static buildMobileRoot(nodes: Record<string, UINode>, rootId: string, width: number, height: number, colors: ExtractedColorSummary) {
    nodes[rootId] = {
      id: rootId,
      type: "page",
      name: "Mobile Page",
  ...
  private static buildDenseMatrixRoot(nodes: Record<string, UINode>, rootId: string, width: number, height: number, colors: ExtractedColorSummary) {
    nodes[rootId] = {
      id: rootId,
      type: "page",
      name: "Matrix Page",
  ...
  private static buildDashboardRoot(nodes: Record<string, UINode>, rootId: string, width: number, height: number, colors: ExtractedColorSummary) {
    nodes[rootId] = {
      id: rootId,
      type: "page",
      name: "Dashboard Page",
  ...
  private static buildFormRoot(nodes: Record<string, UINode>, rootId: string, width: number, height: number, colors: ExtractedColorSummary) {
    nodes[rootId] = {
      id: rootId,
      type: "page",
      name: "Form Page",
  ...
  private static buildLandingRoot(nodes: Record<string, UINode>, rootId: string, width: number, height: number, colors: ExtractedColorSummary) {
    nodes[rootId] = {
      id: rootId,
      type: "page",
      name: "Landing Page",
  ...
  private static buildCardGrid(
    ...
    nodes[`${id}_container`] = {
      ...
      type: "grid",
      parentId: id,
      childIds: items.map((_: any, idx: number) => `${id}_card_${idx}`),
      position: { x: 0, y: 0, relativeTo: "flow" },
  ...
  private static buildForm(
    ...
    for (const inp of inputs) {
      nodes[inp.id] = {
        ...
        content: { placeholder: inp.placeholder, inputType: inp.type as any },
        confidence: 0.98,
      };
    }
