...
import { GeneratedFile, GeneratedProject } from "../types/generator.js";
import { TargetCodeGenerator } from "./types.js";
import { DesignTokenEngine } from "../tokens/engine.js";

export class ReactGenerator implements TargetCodeGenerator {
  public readonly target = "react" as const;
  ...
  public async generate(
    ...
    for (const [compId, compNode] of Object.entries(plan.components)) {
      if (compId === plan.rootComponentId) continue;
...
`,
  ...
  private generateGlobalCss(tokens: DesignTokens, ir: UIIRDocument): string {
...
`;
  ...
  private resolveLucideIcon(name: string): string {
  ...
  private nodeStyleToReactInline(
    ...
    if (layout) {
      ...
      if (layout.display === "flex") {
        ...
        if (layout.flexWrap) inline.flexWrap = layout.flexWrap;
    ...
    if (styles) {
      ...
      if (styles.backgroundGradient) inline.background = styles.backgroundGradient;
      if (styles.color) inline.color = styles.color;
      if (styles.fontFamily) inline.fontFamily = styles.fontFamily;
      if (styles.fontSize) {
      ...
      if (styles.letterSpacing) inline.letterSpacing = styles.letterSpacing;
      if (styles.textAlign) inline.textAlign = styles.textAlign;
      if (styles.textTransform) inline.textTransform = styles.textTransform;
      ...
      if (styles.boxShadow) inline.boxShadow = styles.boxShadow;
      if (styles.backdropFilter) inline.backdropFilter = styles.backdropFilter;
    }
