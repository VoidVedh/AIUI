...
export class ComponentPlanner {
   ...
   */
  public static planComponents(ir: UIIRDocument): ComponentPlan {
    const rootNode = ir.nodes[ir.rootNodeId];
    if (!rootNode) {
      throw new Error(`Root node '${ir.rootNodeId}' not found in UIIRDocument.`);
    }

    const components: Record<string, ComponentPlanNode> = {};
  ...
  private static isSectionComponent(node: UINode): boolean {
    return [
      "navbar",
      "sidebar",
      "header",
  ...
  private static formatComponentName(type: string, name: string): string {
  ...
  private static mapCategory(type: string): ComponentPlanNode["category"] {
    switch (type) {
      case "navbar":
      case "sidebar":
      case "header":
  ...
  private static collectAllDescendants(nodeId: string, nodes: Record<string, UINode>): string[] {
  ...
  private static inferProps(node: UINode, nodes: Record<string, UINode>) {
    const props = [];
  ...
  private static inferState(node: UINode) {
    const state = [];
    if (node.type === "form") {
      state.push({
        name: "formData",
