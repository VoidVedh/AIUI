...
export class DesignTokenEngine {
        ...
        for (const node of Object.values(ir.nodes)) {
            ...
            if (styles.padding) {
                if (styles.padding.top)
                    rawSpacings.add(styles.padding.top);
                if (styles.padding.right)
                    rawSpacings.add(styles.padding.right);
                if (styles.padding.bottom)
                    rawSpacings.add(styles.padding.bottom);
                if (styles.padding.left)
                    rawSpacings.add(styles.padding.left);
            }
            if (styles.margin) {
                if (styles.margin.top)
                    rawSpacings.add(styles.margin.top);
                if (styles.margin.right)
                    rawSpacings.add(styles.margin.right);
                if (styles.margin.bottom)
                    rawSpacings.add(styles.margin.bottom);
                if (styles.margin.left)
                    rawSpacings.add(styles.margin.left);
            }
            if (styles.borderRadius) {
                if (styles.borderRadius.topLeft)
                    rawRadii.add(styles.borderRadius.topLeft);
                if (styles.borderRadius.topRight)
                    rawRadii.add(styles.borderRadius.topRight);
                if (styles.borderRadius.bottomRight)
                    rawRadii.add(styles.borderRadius.bottomRight);
                if (styles.borderRadius.bottomLeft)
                    rawRadii.add(styles.borderRadius.bottomLeft);
            }
        }
    ...
    static toCssVariables(tokens) {
        ...
        for (const [key, value] of Object.entries(tokens.colors)) {
            if (key === "gradients") {
                for (const [gradKey, gradVal] of Object.entries(value)) {
    ...
    static inferPrimaryColor(colors, bg) {
    ...
    static inferSurfaceColor(colors, bg) {
    ...
    static inferSurfaceCardColor(colors, bg) {
        const surface = this.inferSurfaceColor(colors, bg);
        return surface || "#1E293B";
    }
    static inferTextMutedColor(colors, rootText) {
        for (const c of Array.from(colors)) {
    ...
    static inferBorderColor(colors, bg) {
        for (const c of Array.from(colors)) {
    ...
    static inferAccentColor(colors, bg) {
