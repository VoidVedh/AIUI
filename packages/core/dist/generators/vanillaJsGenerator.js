...
export class VanillaJsGenerator {
    target = "vanillajs";
    /**
...
`;
    ...
    renderNodeToHtml(nodeId, nodes, indent) {
    ...
    nodeStyleToCssInline(styles, layout, dimensions) {
        const rules = [];
        if (dimensions) {
        ...
        if (layout) {
            ...
            if (layout.display === "flex") {
                ...
                if (layout.justifyContent)
                    rules.push(`justify-content: ${layout.justifyContent}`);
                if (layout.alignItems)
                    rules.push(`align-items: ${layout.alignItems}`);
                if (layout.gap) {
                    rules.push(`gap: ${typeof layout.gap === "number" ? `${layout.gap}px` : layout.gap}`);
                }
                if (layout.flexWrap)
                    rules.push(`flex-wrap: ${layout.flexWrap}`);
            }
            else if (layout.display === "grid") {
                if (layout.gridTemplateColumns)
                    rules.push(`grid-template-columns: ${layout.gridTemplateColumns}`);
                if (layout.gap) {
                    rules.push(`gap: ${typeof layout.gap === "number" ? `${layout.gap}px` : layout.gap}`);
                }
            }
        ...
        if (styles) {
            ...
            if (styles.backgroundGradient)
                rules.push(`background: ${styles.backgroundGradient}`);
            if (styles.color)
                rules.push(`color: ${styles.color}`);
            if (styles.fontFamily)
                rules.push(`font-family: ${styles.fontFamily}`);
            if (styles.fontSize) {
            ...
            if (styles.boxShadow)
                rules.push(`box-shadow: ${styles.boxShadow}`);
        }
        return rules.join("; ");
    }
    escapeHtml(str) {
...
//# sourceMappingURL=vanillaJsGenerator.js.map