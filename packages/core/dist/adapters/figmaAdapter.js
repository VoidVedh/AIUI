...
export class FigmaInputAdapter {
    supportedType = "figma";
    /**
    ...
    convertFigmaNode(fNode, irId, parentId, nodeMap) {
        ...
        // Typography
        if (fNode.style) {
            if (fNode.style.fontFamily)
                styles.fontFamily = fNode.style.fontFamily;
            if (fNode.style.fontSize)
                styles.fontSize = `${fNode.style.fontSize}px`;
            if (fNode.style.fontWeight)
                styles.fontWeight = fNode.style.fontWeight;
            if (fNode.style.letterSpacing)
                styles.letterSpacing = `${fNode.style.letterSpacing}px`;
            if (fNode.style.lineHeightPx)
                styles.lineHeight = fNode.style.lineHeightPx / (fNode.style.fontSize || 16);
...
//# sourceMappingURL=figmaAdapter.js.map