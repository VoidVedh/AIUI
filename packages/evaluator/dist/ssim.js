...
export class SsimCalculator {
    static C1 = (0.01 * 255) ** 2; // 6.5025
    static C2 = (0.03 * 255) ** 2; // 58.5225
    /**
     ...
     */
    static async compute(image1Buffer, image2Buffer, targetWidth = 640, targetHeight = 400, blockSize = 16, blurSigma = 1.5) {
        ...
        for (let by = 0; by < blocksY; by++) {
            for (let bx = 0; bx < blocksX; bx++) {
                ...
                const blockSsim = Math.max(0, Math.min(1, ((2 * mu1 * mu2 + this.C1) * (2 * sigma12 + this.C2)) /
